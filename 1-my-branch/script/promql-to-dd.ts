...
const logger = new Logger('promql-to-dd');
...
const httpsAgent = new Agent({
  cert: Buffer.from(
    Buffer.from(Env.TEMPORAL_OBSERVABILITY_CERT, 'base64').toString(),
  ),
  key: Buffer.from(
    Buffer.from(Env.TEMPORAL_OBSERVABILITY_KEY, 'base64').toString(),
  ),
});
...
const TEMPORAL_CLOUD_BASE_URL = Env.TEMPORAL_CLOUD_BASE_URL;
...
export const promqlToDD = async () => {
  ...
  while (true) {
    ...
    logger.log('Collecting metrics from temporal cloud.', {
      startDate: new Date(
    ...
    logger.log('Submitting metrics to Datadog');
    await datadogMetricsApi.submitMetrics({
      body: {
        series: [...countSeries, ...guageSeries],
      },
    });

    logger.log('Pausing for 20s');
